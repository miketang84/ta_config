...
keys.cg = m_editing.match_brace  -- modified by tg.
...
keys[not OSX and 'cm' or 'mm'] = m_editing.block_comment
keys.ct = m_editing.transpose_chars
keys[not OSX and 'cJ' or 'cj'] = m_editing.join_lines
-- Select.
keys.cG= { m_editing.match_brace, 'select' }     -- modified by tg.
...
keys[not OSX and 'cL' or 'mL'] = m_editing.select_line  -- added by tg
keys[not OSX and 'ck' or 'mk'] = m_editing.kill_line  -- added by tg
...
keys[not OSX and 'a<' or 'c<'] = utils.enose_as_xml_tags
keys[not OSX and 'a>' or 'c>'] = { m_editing.enose, '<', ' />' }
keys[not OSX and "a'" or "c'"] = { m_editing.enose, "'", "'" }
keys[not OSX and 'a"' or 'c"'] = { m_editing.enose, '"', '"' }
keys[not OSX and 'a(' or 'c('] = { m_editing.enose, '(', ')' }
keys[not OSX and 'a[' or 'c['] = { m_editing.enose, '[', ']' }
...
-- Snippets.
keys[not OSX and 'c0' or 'a\t'] = m_textadept.snippets._select -- modified by tg.
keys['\t'] = m_textadept.snippets._insert
keys['s\t'] = m_textadept.snippets._previous
keys[not OSX and 'c9' or 'as\t'] = m_textadept.snippets._cancel_current  -- modified by tg.
-- Bookmark.
keys[not OSX and 'cf2' or 'mf2'] = m_textadept.bookmarks.toggle
keys[not OSX and 'csf2' or 'msf2'] = m_textadept.bookmarks.ear
...
-- Buffer.
keys['an'] = { _view.goto_buffer, _view, 1, false }    	-- modified by tg.
keys['ap'] = { _view.goto_buffer, _view, -1, false }	-- modified by tg.
...
-- keys[not OSX and '' or 'mL'] = m_textadept.mime_types.select_lexer
